...
class [[clang::lto_visibility_public]] BrokerServices {
 public:
  ...
  // Synchronous wrapper around SpawnTargetAsync. Creates the target process
  // and waits for completion, returning the raw PROCESS_INFORMATION via
  // |target_info|. The caller is responsible for closing the handles.
