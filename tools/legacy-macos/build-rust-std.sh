#!/bin/bash
# Builds a Rust std for x86_64-apple-darwin with a deployment target below
# rustc's 10.12 floor. For 10.6, TLS is emulated (-Ztls-model=emulated)
# because 10.6 has no dyld TLS support; 10.7+ use native TLS. The result is
# installed as a custom sysroot that the rustc wrapper (rustc-emutls /
# rustc-legacy) passes to rustc via --sysroot, so every crate in the build
# compiles and links against the same std.
#
# Usage: tools/legacy-macos/build-rust-std.sh [macos-version]
set -e

ROOT="$(cd "$(dirname "$0")" && pwd)"
MACOS_VERSION="${1:-10.6}"
if [ "$MACOS_VERSION" = "10.6" ]; then
  SYSROOT="$ROOT/dist/rust-sysroot"
  TLS_FLAGS="-Ztls-model=emulated"
  WRAPPER="$ROOT/rustc-emutls"
else
  SYSROOT="$ROOT/dist/rust-sysroot-${MACOS_VERSION}"
  TLS_FLAGS=""
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/rust-std-legacy.XXXXXX")"
...
# sysroot rlibs.
STD_FLAGS="$TLS_FLAGS -Cembed-bitcode=yes"
...
RUSTC_BOOTSTRAP=1 "$WRAPPER" --crate-name panic_abort \
  --crate-type rlib -Cpanic=abort -Copt-level=2 $TLS_FLAGS \
