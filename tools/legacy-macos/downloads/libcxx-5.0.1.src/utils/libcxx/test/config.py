...
class Configuration(object):
    ...
    def configure_features(self):
        ...
        if (
            ...
            and self.cxx.hasCompileFlag(["-x", "objective-c++", "-fobjc-arc"])
        ):
    ...
    def configure_compile_flags_header_includes(self):
        ...
        if self.cxx_stdlib_under_test == "msvc":
            self.cxx.compile_flags += [
                "-include",
                os.path.join(support_path, "msvc_stdlib_force_include.hpp"),
            ]
            pass
        if (
            self.is_windows
    ...
    def configure_link_flags(self):
        ...
        if not no_default_flags:
            ...
            if self.cxx_stdlib_under_test == "libc++":
                ...
                self.lit_config.fatal(
                    'unsupported value for "use_stdlib_type": %s' % use_stdlib_type
                )
    ...
    def configure_warnings(self):
        ...
        default_enable_warnings = (
            self.cxx.type in ["clang", "apple-clang"]
            and len(
                self.config.available_features.intersection(["c++11", "c++14", "c++1z"])
            )
            != 0
        )
