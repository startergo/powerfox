...
/* Vendored from LLVM compiler-rt (emutls.c, Apache-2.0 WITH LLVM-exception)
   with int_lib.h inlined: this build uses -femulated-tls /
   -Ztls-model=emulated because 10.6 has no TLS support, and Apple's clang
   does not ship the emutls runtime. */

#define COMPILE_TIME_ASSERT(e) \
  typedef char compile_time_assert[(e) ? 1 : -1] __attribute__((unused))
